...
void grava_arquivo (ALUNO turma[])
{
	...
	for (i=0; i<max; i++)
		{
			fputs(turma[i].ra, arq);
			fputs("-", arq);
			fputs(turma[i].nome, arq);
			fputs("-", arq);
			fprintf (arq, "%.2f", turma[i].nota1);
			fputs("-", arq);
			fprintf (arq, "%.2f", turma[i].nota2);
			fputs("\n", arq);
		}
...
void ler_arquivo(ALUNO turma[])
{
	...
	for(i=0;i<max;i++)
	{
		fscanf(arq,"%s %[^\n]s %f %f",turma[i].ra, turma[i].nome, &turma[i].nota1, &turma[i].nota2);
